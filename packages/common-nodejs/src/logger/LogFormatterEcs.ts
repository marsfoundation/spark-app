  ...
  public format(entry: LogEntry): string {
    const core = {
      ...
      error: entry.resolvedError
        ? {
            message: entry.resolvedError.error ? truncate(entry.resolvedError.error) : undefined,
            type: entry.resolvedError.name,
            stack_trace: entry.resolvedError.stack ? entry.resolvedError.stack.map(truncate) : undefined,
          }
        : undefined,
    }
