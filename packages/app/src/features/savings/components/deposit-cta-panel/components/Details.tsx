...
export function Details({ description, inputTokens, outputToken }: DetailsProps) {
  return (
    <div className="flex flex-col gap-4">
      <div className={tokensRouteVariants({ bg: outputToken.symbol.toLowerCase() })}>
        <IconStack
...
const tokensRouteVariants = cva<{ bg: Record<string, string> }>('flex w-fit items-center gap-1 rounded-full p-0.5', {
  variants: {
    bg: {
