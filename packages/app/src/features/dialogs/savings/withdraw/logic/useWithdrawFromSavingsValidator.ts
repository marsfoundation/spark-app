...
export function getValidatorConfig({
  ...
  if (chainId === base.id) {
    ...
    return ensureDynamicValidatorConfigTypes({
      ...
      createValidator: ({ psm3UsdcBalance, psm3UsdsBalance }) =>
        getSavingsWithdrawDialogFormValidator({ savingsTokenBalance, savingsConverter }).superRefine((field, ctx) => {
  ...
  return ensureDynamicValidatorConfigTypes({
    ...
    createValidator: () => getSavingsWithdrawDialogFormValidator({ savingsTokenBalance, savingsConverter }),
  })
}
